...
query = """CREATE TABLE IF NOT EXISTS plazas.visit (id_client CHAR(10), id_store VARCHAR, hour TIME, date DATE,  
                         FOREIGN KEY(id_store) REFERENCES plazas.store, FOREIGN KEY(id_client) REFERENCES plazas.client);"""
cur = myConnection.cursor()
cur.execute(query)
cur.close()
myConnection.commit()
...
query = """CREATE TABLE IF NOT EXISTS plazas.shelf_quantity (id_store VARCHAR, id_shelf SERIAL, quantity INT, hour TIME,date DATE,  
                         FOREIGN KEY(id_store, id_shelf) REFERENCES plazas.shelf(id_store,id_shelf));"""
cur = myConnection.cursor()
cur.execute(query)
cur.close()
myConnection.commit()
...
query = """CREATE TABLE IF NOT EXISTS plazas.bill_detail (id_bill INT, id_product INT, price NUMERIC(12,2), quantity INT, 
                        FOREIGN KEY(id_bill) REFERENCES plazas.bill, FOREIGN KEY(id_product) REFERENCES plazas.product);"""
cur = myConnection.cursor()
cur.execute(query)
cur.close()
myConnection.commit()
...
query = """CREATE TABLE IF NOT EXISTS plazas.stored_in (id_warehouse VARCHAR, id_product INT,quantity INT, hour TIME, date DATE,
                    FOREIGN KEY(id_warehouse) REFERENCES plazas.warehouse, FOREIGN KEY(id_product) REFERENCES plazas.product);"""
cur = myConnection.cursor()
cur.execute(query)
cur.close()
myConnection.commit()
...
query = """CREATE TABLE IF NOT EXISTS plazas.thermometer (id_store VARCHAR, id_thermometer VARCHAR, mintemp INT, maxtemp INT,
                        PRIMARY KEY (id_store, id_thermometer), FOREIGN KEY(id_store) REFERENCES plazas.store);"""
cur = myConnection.cursor()
cur.execute(query)
cur.close()
myConnection.commit()
...
query = """CREATE TABLE IF NOT EXISTS plazas.ther_register (id_store VARCHAR, id_thermometer VARCHAR, temperature INT, hour TIME, date DATE,
                         FOREIGN KEY(id_store,id_thermometer) REFERENCES plazas.thermometer);"""
cur = myConnection.cursor()
cur.execute(query)
